...
class ChartArancel extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.chart.chart-arancel', compact('chart', 'tabla'));
    }
    ...
    {
        $data = Declaracion2022::select('subpartida', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
    ...
    {
        $data = DeclaracionEcuador::select('subpartida', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
            ->operacion($this->operacion($this->operacion))
            ->rango($this->desde, $this->hasta)
            ->distrito($this->distrito)
            ->iva($this->iva)
    ...
    {
        $data = Decreto::select('subpartida', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
            ->operacion($this->operacion($this->operacion))
            ->rango($this->desde, $this->hasta)
            ->distrito($this->distrito)
            ->iva($this->iva)
