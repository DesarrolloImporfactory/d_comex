...
class ChartAgente extends Component
{
    ...
    public function render()
    {
        ...
        foreach ($data as $consulta) {
            $array[] = [
                'name' => $consulta['agente_afianzado'],
                'y' => $consulta['cantidad_declaraciones']
            ];
        }

        $chart = json_encode($array);
        return view('livewire.chart.chart-agente', compact('chart', 'tabla'));
    }
    public function declaracion_2022($request)
    {
        $data = Declaracion2022::operacion($this->operacion($this->operacion))->rango($this->desde, $this->hasta)->distrito($this->distrito)
            ...
            ->select('agente_afianzado', DB::raw('COUNT(*) as cantidad_declaraciones'))
    ...
    public function declaracion22($request)
    {
        ...
        $data = Declaracion2022::select('agente_afianzado', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
    ...
    public function declaracion23($request)
    {
        ...
        $data = DeclaracionEcuador::select('agente_afianzado', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
            ->operacion($this->operacion($this->operacion))
            ->rango($this->desde, $this->hasta)
            ->distrito($this->distrito)
            ->iva($this->iva)
    ...
    public function declaracion24($request)
    {
        ...
        $data = Decreto::select('agente_afianzado', DB::raw('COUNT(*) as cantidad_declaraciones'), DB::raw('SUM(fob) as total_fob'), DB::raw('SUM(cif) as total_cif'))
            ->operacion($this->operacion($this->operacion))
            ->rango($this->desde, $this->hasta)
            ->distrito($this->distrito)
            ->iva($this->iva)
